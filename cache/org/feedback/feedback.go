...
var cacheFeedbackLog = mlog.NewTagLog("cache_feedback")

func Handle(o *oev.ObjectEvent) error {
	defer runtime.Recover()
	pos, _ := o.Data.(*feedback.Feedback)
	cacheFeedbackLog.Infof(0, "Handle Event Feedback %s", o.Action)
	switch o.Action {
...
func Set(k string, v *feedback.Feedback) error {
...
func Get(k string) (*feedback.Feedback, error) {
	var c = &feedback.Feedback{}
	var redisdb = redis.GetRedisClient()
	val, err := redisdb.Get(k).Result()
	if err != nil {
		com, err := feedback.GetByID(k)
		if err != nil {
			return nil, err
		}
		Set(k, com)
...
func Update(k string, v *feedback.Feedback) error {
	_ = Del(k)
	err := Set(k, v)
	return err
}
