...
var cacheVideoLog = mlog.NewTagLog("cache_video")

func Handle(o *oev.ObjectEvent) error {
	defer runtime.Recover()
	pos, _ := o.Data.(*video.Video)
	cacheVideoLog.Infof(0, "Handle Event Video %s", o.Action)
	switch o.Action {
...
func Set(k string, v *video.Video) error {
...
func Get(k string) (*video.Video, error) {
	var c = &video.Video{}
	var redisdb = redis.GetRedisClient()
	val, err := redisdb.Get(k).Result()
	if err != nil {
		com, err := video.GetByID(k)
		if err != nil {
			return nil, err
		}
		Set(k, com)
...
func Update(k string, v *video.Video) error {
	_ = Del(k)
	err := Set(k, v)
	return err
}
