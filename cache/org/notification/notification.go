...
var cacheRankLog = mlog.NewTagLog("cache_notification")

func Handle(o *oev.ObjectEvent) error {
	defer runtime.Recover()
	ran, _ := o.Data.(*notification.NotifiationManagement)
	cacheRankLog.Infof(0, "Handle Event Notification %s", o.Action)
	switch o.Action {
...
func Set(k string, v *notification.NotifiationManagement) error {
...
func Get(k string) (*notification.NotifiationManagement, error) {
	var c = &notification.NotifiationManagement{}
	var redisdb = redis.GetRedisClient()
	val, err := redisdb.Get(k).Result()
	if err != nil {
		com, err := notification.GetByID(k)
		if err != nil {
			return nil, err
		}
		Set(k, com)
...
func Update(k string, v *notification.NotifiationManagement) error {
	_ = Del(k)
	err := Set(k, v)
	return err
}
