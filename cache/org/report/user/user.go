...
func Handle(o *oev.ObjectEvent) error {
	...
	usr, _ := o.Data.(*user.UserReport)
	cacheUserLog.Infof(0, "Handle Event UserReport %s", o.Action)
	switch o.Action {
...
func Set(k string, v *user.UserReport) error {
...
func Get(k string) (*user.UserReport, error) {
	var c = &user.UserReport{}
	var redisdb = redis.GetRedisClient()
...
func Update(k string, v *user.UserReport) error {
	_ = Del(k)
	err := Set(k, v)
	return err
}
